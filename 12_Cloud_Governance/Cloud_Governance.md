...
| **SCP (Service Control Policy)** | Chính sách kiểm soát *những gì có thể làm* — áp lên OU/Account |
| **RCP (Resource Control Policy)** | Kiểm soát *ai có thể truy cập tài nguyên* từ bên ngoài account |
...
**Giải thích:**
- **SCP** = kiểm soát *hành động của người dùng* trong account (Identity-side).
- **RCP** = kiểm soát *ai được phép truy cập tài nguyên* bất kể họ ở đâu (Resource-side).
...
stateDiagram-v2
    ...
    Recording --> Evaluating: Config Rule đánh giá\ncấu hình hiện tại
    Evaluating --> COMPLIANT: Tuân thủ ✅
    Evaluating --> NON_COMPLIANT: Vi phạm ❌
    NON_COMPLIANT --> Remediation: Auto Remediation\n(SSM Automation / Lambda)
    Remediation --> COMPLIANT: Đã sửa
    NON_COMPLIANT --> Notification: SNS Alert\ncho đội bảo mật
    COMPLIANT --> [*]
```
